...
public class IconNodeInst extends NodeInst {
     ...
     * @param key the key of the parameter
     * @return true if the key is a defined parameter of this IconNodeInst
     */
    @Override
...
//
   ...
   /**
    ...
    public int getNumNodables() {
        if (nodables == null) {
            Cell parent = getParent();
            if (parent != null && !getParent().isIcon() && !getParent().isSchematic()) {
                return 1;
            }
            if (isIconOfParent()) {
                return 0;
    ...
    private class IconNodable implements Nodable {
         ...
         * @return true because NetSchem objects are always cell instances.
         */
        @Override
