...
public class JELIB extends LibraryFiles
{
    ...
    public static Map<Setting,Object> readProjectSettings(URL fileURL, FileType fileType, TechPool techPool, ErrorLogger errorLogger) {
        ...
        for (Map.Entry<TechId,Variable[]> e: parser.techIds.entrySet()) {
            ...
            if (tech == null) {
                errorLogger.logError(fileURL +
                    ", Cannot identify technology " + techId.techName + " while reading project settings", -1);
                continue;
            }
    ...
    Map<Cell,Variable[]> createLibraryCells(boolean onlyProjectSettings, EditingPreferences ep) {
        ...
        for (Map.Entry<TechId,Variable[]> e: parser.techIds.entrySet()) {
            ...
            if (tech == null) {
            	String msg = (filePath != null) ? filePath + ", ": "";
                Input.errorLogger.logError(msg +
        ...
        for (Map.Entry<LibId,String> e: parser.externalLibIds.entrySet()) {
            ...
            if (Library.findLibrary(libId.libName) == null)
                readExternalLibraryFromFilename(libFileName, fileType, ep);
        }
        ...
        for (JelibParser.NodeContents n: cc.nodes) {
            ...
            if (protoId instanceof CellId)
                ...
                if (np == null) {
                    ...
                    Set<Object> primSet = undefinedTechsAndPrimitives.get(pnId.techId);
                    if (primSet == null)
                    {
                        primSet = new HashSet<Object>();
			...
			if (np == null)
			{
                ...
                if (dummyCell != null && dummyCell.getVar(IO_DUMMY_OBJECT) == null)
                    dummyCell = null;
                if (dummyCell == null) {
                    dummyCell = Cell.makeInstance(ep, cellLib, protoName);
                    if (dummyCell == null)
                    {
                        Input.errorLogger.logError(cc.fileName + ", line " + line +
				...
				dummyCell.newVar(IO_TRUE_LIBRARY, prefixName, ep);
				dummyCell.newVar(IO_DUMMY_OBJECT, protoName, ep);
				np = dummyCell;
			}
...
//            }
        ...
        for (JelibParser.ExportContents e: cc.exports) {
			...
			PortInst pi = figureOutPortInst(cell, e.originalPort.externalId, (JelibParser.NodeContents)e.originalNode, e.pos, cc.fileName, line);
			if (pi == null) continue;

			// create the export
			Export pp = Export.newInstanceNoIcon(cell, e.exportId, e.exportUserName, e.nameTextDescriptor, pi,
		...
		for(JelibParser.ArcContents a: cc.arcs) {
			...
			if (ap == null)
            {
                ...
                if (ap == null)
                {
                    ...
                    Set<Object> arcSet = undefinedTechsAndPrimitives.get(arcId.techId);
                    if (arcSet == null)
                    {
                        arcSet = new HashSet<Object>();
			...
			PortInst headPI = figureOutPortInst(cell, a.headPort.externalId, (JelibParser.NodeContents)a.headNode, a.headPoint, cc.fileName, line);
			if (headPI == null) continue;

			PortInst tailPI = figureOutPortInst(cell, a.tailPort.externalId, (JelibParser.NodeContents)a.tailNode, a.tailPoint, cc.fileName, line);
            ...
            ArcInst ai = ArcInst.newInstanceNoCheck(cell, ap, a.arcName, nameDescriptor,
            ...
            if (!ai.getName().equals(a.arcName) && (a.arcName != null && a.arcName.indexOf('@') < 0) && ai.getVar(Artwork.ART_MESSAGE) == null) {
                if (a.nameTextDescriptor != null) {
                    ...
                    ai.newDisplayVar(Artwork.ART_MESSAGE, a.arcName, ep);
                }
...
//		}
		...
		Export export = Export.newInstanceNoIcon(subCell, portPI, name, ep, null);
		if (export == null)
		{
			Input.errorLogger.logError(fileName + ", line " + lineNumber +
