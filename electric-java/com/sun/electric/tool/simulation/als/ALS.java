...
{
    /** initial size of simulation window: 200ns */	private static final double DEFTIMERANGE = 0.0000002;
	...
	static class Model
	{
		String          name;
		char            type;
		Object          ptr;	/* may be Connect, Row, or Function */
	...
	static class UserProc
	{
		 ...
		 *
		 * @param nodeHead pointer to the node data structure to be updated
		 * @param operator char indicating operation to be performed on node
		 * @param state integer value representing the state of the node
		 * @param strength integer value representing the logic strength of the signal
		 * @param time double value representing the time the change is to take place
		 */
		protected void scheduleNodeUpdate(Model primHead, ALSExport exHead, int operator,
			Object state, int strength, double time)
		{
			Stat statHead = (Stat)exHead.nodeName;
			if (statHead.schedOp == operator && statHead.schedState == state &&
			...
			linkPtr2.operatr = statHead.schedOp = (char)operator;
		 ...
		 *
		 * @param primHead pointer to current bidirectional element
		 * @param side pointers to nodes on each side of the bidirectional element
		 * @param outStrength output strength
		 */
...
//    @Override
	 ...
	 */
	public double getMinTimeRange() { return DEFTIMERANGE; }
...
//		new UserCom.Bus12ToState();
	 ...
	 * for the simulation.
	 * @param linkHead pointer to the data element that is going to be inserted
	 */
...
//            sig.addSample(DEFTIMERANGE, DigitalSample.LOGIC_0);
	 ...
	 * @param line pointer to the character string to be fragmented
	 */
	 ...
	 * @param sp pointer to char string containing path name to level in xref table
	 */
	 ...
	 * @param nodeHead pointer to desired node in database
	 * @param sp pointer to char string where complete name is to be saved
	 */
	 ...
	 * @param cellHead pointer to desired level of hierarchy
	 * @param sp pointer to char string where path name is to be saved
	 */
	 ...
	 * @param sp pointer to char string containing path name to node
	 */
