					...
					if (ni.isCellInstance())
					{
						...
						if (var != null)
						{
							...
							if (subLib == null && recursively)
							{
								...
								{
									subLib.newVar(PROJPATHKEY, projFile, ep);
									...
									for(Iterator<ProjectCell> pIt = subPL.getProjectCells(); pIt.hasNext(); )
									{
										...
										{
											getCellFromRepository(pdb, recPC, subLib, true, report, ep);
											if (recPC.getCell() == null)
										...
										if (recPC.getCell() != null)
										{
											...
											markLocked(recPC.getCell(), !youOwn, ep);		// CHANGES DATABASE
										}
									}
								}
							}
						...
						if (realSubCell == null && recursively)
						{
							...
							if (subPC != null)
							{
								...
								getCellFromRepository(pdb, subPC, subLib, recursively, false, ep);
...
//		}
	...
	static boolean writeCell(Cell cell, ProjectCell pc, int backupScheme, EditingPreferences ep) throws JobException
	{
		...
		Cell cellCopy = copyRecursively(cell, fLib, ep);		// CHANGES DATABASE
		if (cellCopy == null)
		{
...
//		fLib.setCurCell(cellCopy);
	...
	private static Cell copyRecursively(Cell fromCell, Library toLib, EditingPreferences ep)
	{
		...
		for(Iterator<NodeInst> it = fromCell.getNodes(); it.hasNext(); )
		{
			...
			if (oCell == null)
			{
				oCell = Cell.makeInstance(ep, toLib, subCellName);
				if (oCell == null)
				{
				...
				if (cell.getLibrary() != fromCell.getLibrary())
					oCell.newVar(PROJLIBRARYKEY, cell.getLibrary().getName(), ep);
