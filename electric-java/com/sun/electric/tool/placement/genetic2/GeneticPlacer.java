...
public class GeneticPlacer {
	// private static final int MAX_INDIVIDUALS = 200; // for islands
	// (deprecated)
	private static final int INDIVIDUALS = 1000; // population size for
													// UnifiedPopulation
	...
	GeneticPlacer(List<PlacementNode> nodesToPlace, List<PlacementNetwork> allNetworks, int runtime,
			int threads, boolean debug) {
		maxRuntime = runtime * 1000;
		EVOLUTION_STEPS = Math.max(maxRuntime / 10000, 1);
		numThreads = threads;
		...
		population = new UnifiedPopulation(nodesToPlace, allNetworks, indis, maxRuntime / EVOLUTION_STEPS,
				numThreads, rand);

	}

	private void placeUnified(List<PlacementNode> nodesToPlace, List<PlacementNetwork> allNetworks,
			long partTime) {
		long localStartTime = System.currentTimeMillis();

		if (printDebugInformation) {
			System.out.println("Starting genetic algorithm...\n" + "Total running time is " + maxRuntime
					/ 1000 + " seconds (~" + maxRuntime / 60000 + " minutes).\n" + "There will be "
					+ EVOLUTION_STEPS + " text messages while running,\n"
		...
		while (t < partTime) {
			...
			population.evolveLocalMT(numThreads); // this is where the evolution
													// happens!

			champion = (DeltaIndividual) population.getChampion();

			if (printDebugInformation) {
				System.out.println("Round " + round + " of " + EVOLUTION_STEPS + " | champion badness: "
						+ population.getChampion().getBadness());
		...
		if (printDebugInformation) {
			...
			// System.out.println("final evolverSteps: " +
			// population.getEvolverSteps());
		}
	}

	private void placeAnnealing(List<PlacementNode> nodesToPlace, List<PlacementNetwork> allNetworks,
			long partTime) {
		long localStartTime = System.currentTimeMillis();
		SimulatedAnnealing annealing = new SimulatedAnnealing(nodesToPlace, allNetworks, maxRuntime
				/ EVOLUTION_STEPS, numThreads, ref);
	...
	// /**
	// * This function is deprecated, use placeUnified or placeAnnealing
	// instead.
	// * It is only provided for comparison and can be deleted at any time
	// */
	// private void placeIslands(List<PlacementNode> nodesToPlace,
	...
	// for(int i=0; i<numThreads; i++) {
	//			
	// ClassicIndividual[] indis = new
	// ClassicIndividual[MAX_INDIVIDUALS/numThreads];
	...
	// allNetworks,
	// indis,
	// (long)(maxRuntime/EVOLUTION_STEPS), // Gesamtzeit geteilt durch
	// Evolutionsschritte
	// rands[i]);
	// }
	//		
	...
	// {
	// System.out.println("Starting genetic algorithm...\n"
	// + "Total running time is " + maxRuntime/1000 + " seconds (~"
	// +maxRuntime/60000+ " minutes).\n"
	...
	// System.out.println("final champion overlap: " +
	// champion.calculateOverlap());
	// System.out.println("final champion semiperimeter length: " +
	// champion.getSemiperimeterLength());
	// System.out.println("final champion bounding box area: " +
	// champion.getBoundingBoxArea());
	//			
