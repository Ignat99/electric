...
public class PlacementSimulatedAnnealing extends PlacementFrame {
	 ...
	 */
	public void runPlacement(List<PlacementNode> nodesToPlace, List<PlacementNetwork> allNetworks, List<PlacementExport> exportsToPlace, String cellName, Job job) {
		if (nodesToPlace.size() < 2) return;
	...
	class SimulatedAnnealing extends Thread {
		...
		private double metricForNode(ProxyNode node) {
			...
			{
				Double nl = netLengths.get(net);
				if (nl != null) metric += nl.doubleValue();
			}
			return metric;
		}
	}
