...
public class BinaryTree {
	...
	private void drawExternalArc(Cell c, Map<Node, NodeInst> nodeToInst) {
		...
		NodeInst dot = LayoutLib.newNodeInst(DOT_ICON_PROTO, ep, dotX, dotY, 
                                             DEF_SIZE, DEF_SIZE, 
                                             0, c);
	...
	private void drawArc(PortInst p1, PortInst p2) {
		if (SLANTED_EDGES) {
			ArcInst aL = ArcInst.makeInstance(LINE_PROTO, ep, p1, p2);
			aL.setFixedAngle(false);
		} else {
			LayoutLib.newArcInst(LINE_PROTO, ep, DEF_SIZE, p1, p2);
		}
	}
	...
	public void draw(Cell c) {
		...
		for (Node n : slots) {
			...
			double x = s * SLOT_WID;
			double y = h * SLOT_HEI;
			NodeInst ni = LayoutLib.newNodeInst(SCH_ICON_PROTO, ep, x, y, 
					                            DEF_SIZE, DEF_SIZE, 
					                            270, c);
			addId(ni, n.getId());
