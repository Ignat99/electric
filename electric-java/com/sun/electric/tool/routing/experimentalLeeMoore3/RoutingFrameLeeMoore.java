...
public class RoutingFrameLeeMoore extends BenchmarkRouter {
	...
	private Map<RoutingLayer, RTNode<LMBound>> metalBlockages;
	private Map<RoutingLayer, RTNode<LMVia>> viaBlockages;
	...
	protected void runRouting(Cell cell, List<RoutingSegment> segmentsToRoute,
		...
		this.metalBlockages = new HashMap<RoutingLayer, RTNode<LMBound>>();
		this.viaBlockages = new HashMap<RoutingLayer, RTNode<LMVia>>();
	...
	private class Wavefront {
		...
		private void propagate() {
			...
			RTNode<LMBound> blockages = metalBlockages.get(this.rlStart);
	...
	private LMBound getMetalBlockage(int netID, int metNo, double halfWidth,
		...
		RTNode<LMBound> rtree = metalBlockages.get(layer);
		if (rtree == null)
			return null;
		...
		try {
			...
			for (Iterator<LMBound> sea = new RTNode.Search<LMBound>(searchArea, rtree, true); sea
					.hasNext();) {
				LMBound sBound = sea.next();
	...
	private void addRectangle(Rectangle2D bounds, RoutingLayer layer, int netID) {
		RTNode<LMBound> root = this.metalBlockages.get(layer);
		if (root == null) {
			root = RTNode.makeTopLevel();
			this.metalBlockages.put(layer, root);
		}
		RTNode<LMBound> newRoot = RTNode
	...
	private void addVia(Point2D loc, RoutingLayer layer, int netID) {
		RTNode<LMVia> root = this.viaBlockages.get(layer);
		if (root == null) {
			root = RTNode.makeTopLevel();
			this.viaBlockages.put(layer, root);
		}
		RTNode<LMVia> newRoot = RTNode.linkGeom(null, root, new LMVia(loc, netID));
		if (newRoot != root)
	 ...
	 * structure.
	 */
	static class LMBound implements RTBounds {
