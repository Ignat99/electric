...
public class RoutingArray {
    ...
    private final boolean USE_MULTI_TERMINAL_ALGORITHM=false;	//multi-terminal algorithm is not yet doing correct wires in electric
    ...
    /**
     * mark tupels as used using a value
    ...
    */
     ...
     * @param r Rectangle
     * @param layer Layer containing blockage
     */
...
//    }
    ...
    /**
     * this method is only used for compatibility with single threaded algorithm
     * @param start
     * @param end
     * @param netID
     */
    ...
    public Route route(Tupel start, Tupel end, ThreadBorders border, int netID) {
        ...
        if (USE_MULTI_TERMINAL_ALGORITHM) {
            if (getValue(start) > RESERVED_POINT) {
                //start point was already routed once, multi-terminal algorithm can not use it as start point
                if (getValue(end) > RESERVED_POINT) {
                    //same for end point, so don't use multi-terminal algorithm
                ...
                //none of the points was already routed once so don't use multi-terminal algorithm to make sure that both will be connected
                useMultiTerminalAlgorithm = false;
            }
        } else {
            useMultiTerminalAlgorithm = false;
    ...
    private void prepareStartEndPoint(Tupel start, Tupel end, ThreadBorders border) {
        ...
        //prepare neighbors (were marked as reserved) if start point is original start point of a routing segment
        if (isStartEndTupel(start)) {
     ...
     * @param multiTerminal use multi-terminal algorithm
    ...
    /**
     * Return value of left neighbor
     * @param t tupel which neighbors to check
     * @param b borders
     * @return value of left neighbor or blocked if out of borders
     */
    ...
    /**
     * Return value of right neighbor
     * @param t tupel which neighbors to check
     * @param b borders
     * @return value of right neighbor or blocked if out of borders
     */
    ...
    /**
     * Return value of bottom neighbor
     * @param t tupel which neighbors to check
     * @param b borders
     * @return value of bottom neighbor or blocked if out of borders
     */
    ...
    /**
     * Return value of top neighbor
     * @param t tupel which neighbors to check
     * @param b borders
     * @return value of top neighbor or blocked if out of borders
     */
    ...
    /**
     * Return value of layer up neighbor
     * @param t tupel which neighbors to check
     * @param b borders
     * @return value of layer up neighbor or blocked if out of borders
     */
    ...
    /**
     * Return value of layer down neighbor
     * @param t tupel which neighbors to check
     * @param b borders
     * @return value of layer down neighbor or blocked if out of borders
     */
     ...
     * (This tupel is only different if multi-terminal is true, which is not by default)
     */
    class SearchResult{
    	Tupel foundEnd;
    	int length;
