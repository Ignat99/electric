...
public class RegionGoal implements AStarGoalBase<AStarRegionNode>
{
   ...
   */
  private static final int VIA_PENALTY = 5;
  ...
  {
    return 10 * (Math.abs(startX - endX) + Math.abs(startY - endY) + (Math.abs(startZ - endZ)));
  }
  ...
  public int getNodeCost(AStarRegionNode from, int toX, int toY, int toZ)
  {
    ...
    if (from.getY() == toY)
    {
      ...
      return (distance(from.getX(), from.getY(), from.getZ(), toX, toY, toZ) + from.getVerticalCapacity() + bendPenalty + viaPenalty);
    }
    else
    // Moving vertically
    if (from.getX() == toX)
    {
      ...
      if (from.origin != null)
        if (from.origin.getX() != toX)
          bendPenalty = BEND_PENALTY;
      return (distance(from.getX(), from.getY(), from.getZ(), toX, toY, toZ) + from.getHorizontalCapacity() + bendPenalty + viaPenalty);
    }
    else
    {
      /*
