...
public class Utils {
	 ...
	 *
	 * @return true if the function can be used as a substrate tap.
	 */
	 ...
	 *
	 * @return true if the function can be used as a well tap.
	 */
	 ...
	 * @param threadIndex
	 */
	public static void spreadWellSeed(double cX, double cY, NetValues wellNum, RTNode<WellBound> rtree,
			int threadIndex) {
		RTNode<WellBound> allFound = null;
	 ...
	 * @param keepSearching
	 * @param threadIndex
	 * @return the RTNode found in the area.
	 */
	public static RTNode<WellBound> searchInArea(Rectangle2D searchArea, NetValues wellNum, RTNode<WellBound> rtree,
			RTNode<WellBound> allFound, Point2D ctr, MutableBoolean keepSearching, int threadIndex) {
		...
		for (Iterator<WellBound> sea = new RTNode.Search<WellBound>(searchArea, rtree, true); sea.hasNext();) {
			...
			if (allFound == null) {
				...
				for (Iterator<WellBound> subSea = new RTNode.Search<WellBound>(wb.getBounds(), allFound, true); subSea
						.hasNext();) {
					WellBound subWB = subSea.next();
