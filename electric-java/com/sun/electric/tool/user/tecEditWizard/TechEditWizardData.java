...
public class TechEditWizardData
{
    ...
    private static int[] nullPattern = new int[] {0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    ...
    private boolean shortNameFlag = false; // to control the name associated to metals/vias
    private boolean defaultSquareContacts = false; // to control when to define automatically those square contacts
    ...
    private List<WideWizardField> wide_metal_spacing = new ArrayList<WideWizardField>(); // For all wide spacing rules not displayed in graphics
	...
	// Extra variable to facilitate parsing
    ...
    public static class RectLayerNode extends LayerNode
    {
        ...
        WizardField inline; // inline spacing value for cuts
        ...
        public RectLayerNode(String l, double valX, String nameX, double valY, String nameY,
        		double inlineV, String nameI)
        {
            super(l);
            valueX = new WizardField(valX, nameX);
            valueY = new WizardField(valY, nameY);
            inline = new WizardField(inlineV, nameI);
        }
    }
    
    public static class NotCenterRectLayerNode extends RectLayerNode
    {
        double valueXR; // overhang or size X value. Size value for cuts
        double valueYT; // overhang or size Y value. Size value for cuts
        
    	public NotCenterRectLayerNode(String l, double valX, double valXR, String nameX, 
    			double valY, double valYR, String nameY)
    	{
    ...
    public static class Element
    {
        ...
        boolean sizeByCutLayer; // to determine if SizeOff is just the cut size

        // odd metals go vertical
        public Element (String p, boolean size)
        {
            prefix = p;
            layers = new ArrayList<LayerNode>();
            sizeByCutLayer = size;
        }
        public List<LayerNode> getLayers() { return layers; }
    }
    ...
    private static Map<String,Xml.ArcProto> allArcs; // to know if special contacts need them
    private static Map<String,String> allNodeNames; // to know if nodenames are unique
    ...
    private static class PaletteGroup
    {
        ...
        private void add(List<Xml.MenuNodeInst> list, Xml.PrimitiveNodeGroup element, String shortName)
        {
            ...
            if (allNodeNames.get(pn.name) != null)
            {
            	System.out.println("ERROR: node '" + pn.name + "' already defined. Skipping newest definition");
            	return;
            }
            allNodeNames.put(pn.name, pn.name);
            if (shortName != null)
            {
                n.text = shortName;
            }
	...
	// extra support class to get valid colors
	...
	private class ColorProperties 
	{
		...
		{
			colourMap = Collections.unmodifiableMap(populateColourMap(classWithColorFields));
		}
		public Color getColor(String key) {return colourMap.get(key);}
		...
		private boolean isPublicStaticFinalColor(Field f) 
		{
			int modifiers = f.getModifiers();

			return Modifier.isPublic(modifiers) &&  Modifier.isStatic(modifiers) &&
					Modifier.isFinal(modifiers) && isColor(f.getType());
		}
    ...
    public static class LayerInfo
    {
        ...
        double width = 0; // width of the pure layer node to create. Initial width
        double height = -1; // height for 3D view
        double thickness = -1; // thickness for 3D view
        boolean addArc = false;
        WizardField spacing, minimum; // spacing and min width rule
        Layer.Function function = Layer.Function.ART; // ART is better default than UNKNOWN
        int functionExtras = 0; // for nonelectrical or p/n diff, etc
        ...
        {
        	this(n);
        	gdsData = new GDSLayers(v, -1, -1, -1, -1, -1);
        }
        String getString(GDSLayerType type) 
        {
        	if (gdsData == null) return ""; // no info
        	return gdsData.getString(type);
        }
        public void setArc(boolean withArc) { this.addArc = withArc; }
        public void setGDSData(String val) { gdsData = GDSLayers.parseLayerString(val); }
        
        public Layer.Function setLayerFunction(String funcName)
        {
        	Layer.Function func = Layer.Function.UNKNOWN;
        	int f = 0;
        	//String fucN = funcName.toUpperCase(); // easy to compare
        	if (funcName.startsWith("VIA")) // via case
        	{
        	...
        	else if (funcName.startsWith("POLYCONT") || funcName.equals("CO")) // contact case 
        		func = Layer.Function.CONTACT1;
        	else if (funcName.startsWith("PW")) // WELLP
        		func = Layer.Function.WELLP;
        	else if (funcName.startsWith("NW")) // WELLN
        		func = Layer.Function.WELLN;
        	else if (funcName.startsWith("OD")) // OD
        		func = Layer.Function.DIFF;
        	else if (funcName.startsWith("PDIFF")) // DIFFP
        		func = Layer.Function.DIFFP;
        	else if (funcName.startsWith("NDIFF")) // DIFFN
        		func = Layer.Function.DIFFN;
        	else if (funcName.equals("PO")) // POLY1
        		func = Layer.Function.POLY1;
        	else if (funcName.equals("NP")) // NPlus
        		func = Layer.Function.IMPLANTN;
        	else if (funcName.equals("PP")) // PPlus
        		func = Layer.Function.IMPLANTP;
        	else if (funcName.equals("VTH_P") ||funcName.equals("VTL_P")) // VTH_P or VTL_P
        	{
        		func = Layer.Function.IMPLANTP;
        		f = Layer.Function.HLVT;
        	}
        	else if (funcName.equals("VTH_N") || funcName.equals("VTL_N")) // VTH_N or VTL_P
        	{
        		func = Layer.Function.IMPLANTN;
        		f = Layer.Function.HLVT;
        	}
        	else if (funcName.equals("PRBOUND")) // Prboundary
        		func = Layer.Function.CONTROL;
        	else if (funcName.equals("RPO")) // RPO
        		func = Layer.Function.ART;
        	else if (funcName.equals("TEXT")) // text
        	{
        		func = Layer.Function.ART;
        		f = Layer.Function.NONELEC;
        	}
        	else
        ...
        public void setColor(String name)
        {
        	...
        	graphicsColor = fromColor.getColor(n);
        }
        
        public void setStyle(Outline style) { graphicsOutline = style; }
        public void setPattern(int patCode) { graphicsPattern = getPatternByCode(patCode); }
        ...
        private void setLayerInformation(String s, LineNumberReader lineReader)
        {
            ...
            while (p.hasMoreTokens())
            {
                ...
                if (str.startsWith("G"))
                {
                    setGDSData(str.substring(index+1));
                }
                else if (str.startsWith("W")) // width
                {
                	validateIndex(index, s);
                ...
                else if (str.startsWith("M")) // min width rule
                {
                	validateIndex(index, s);
                ...
                {
                	validateIndex(index, s);
                    function = Layer.Function.findFunction(str.substring(index+1)); // Layer.Function.valueOf(str.substring(index+1));
                }
                else if (str.startsWith("E")) // extras for function
                {
                	validateIndex(index, s);
                	functionExtras = Layer.Function.parseExtraName(str.substring(index+1)); // 
                }
                ...
                {
                	validateIndex(index, s);
                    setGraphicsTemplate(str.substring(index+1), lineReader);
                }
                else
                {
                    System.out.println("Case not implemented str='" + str + "', in line " + lineReader.getLineNumber());
                    assert(false);
                }
            }
        }

        public void setGraphicsTemplate(String s, LineNumberReader lineReader)
        {
            if (s.startsWith("[")) // color
            {
                ...
                if (outlineOrPattern != null)
                {
                    ...
                    else
                    {
                    	...
                    	{
                    		if (lineReader != null)
                    			System.out.println("Systax error in line '" + outlineOrPattern + "', in line " + lineReader.getLineNumber());
                    		else
                    			System.out.println("Systax error in " + s);
                    		if (Job.getDebug())
        ...
        public String toString() { return (gdsData != null) ? gdsData.toString() : ""; }
        String getLayerNumber(GDSLayerType type) { return (gdsData != null) ?  ""+gdsData.getLayerNumber(type): ""; }
    }
    ...
    private LayerInfo marking_layer = new LayerInfo("DeviceMark", -1);		// Device marking layer, 100 might not be the right value
    private List<LayerInfo> extraLayers; // extra layers
    private List<LayerInfo> textLayers; // text layers; they are in the same palette group.
    ...
    LayerInfo[] getBasicLayers()
    {
        ...
        layers.addAll(textLayers); // nothing added if list is empty 
    ...
    public TechEditWizardData(LineNumberReader r)
	{
		stepsize = 100;
        num_metal_layers = 2;
        allocateVariables();
    ...
    {
    	// to support color search
    	if (fromColor == null)
    		fromColor = new ColorProperties(Color.class);
    	
    	// Garbage collector will deal with previously allocated variables.
    	// Not worth it to clear them
		metal_width = new ArrayList<WizardField>(num_metal_layers);
		metal_spacing = new ArrayList<WizardField>(num_metal_layers);
        metal_minarea = new ArrayList<WizardField>(num_metal_layers);
        metal_enclosedarea = new ArrayList<WizardField>(num_metal_layers);
        int numVias = (num_metal_layers > 0) ? num_metal_layers - 1 : 0;
        via_size = new ArrayList<WizardField>(numVias);
        via_inline_spacing = new ArrayList<WizardField>(numVias);
        via_array_spacing = new ArrayList<WizardField>(numVias);
        via_overhang = new ArrayList<WizardField>(numVias);
		metal_antenna_ratio = new double[num_metal_layers];
        metal_layers = new LayerInfo[num_metal_layers];
		via_layers = new LayerInfo[numVias];

        for(int i=0; i<num_metal_layers; i++)
		{
			metal_width.add(new WizardField());
			metal_spacing.add(new WizardField());
            metal_minarea.add(new WizardField());
            metal_enclosedarea.add(new WizardField());
            
            metal_layers[i] = new LayerInfo(getMetalName()+(i+1));
        }

        for(int i=0; i<numVias; i++)
		{
        	via_size.add(new WizardField());
        	via_inline_spacing.add(new WizardField());
        	via_array_spacing.add(new WizardField());
        	via_overhang.add(new WizardField());
            via_layers[i] = new LayerInfo(getViaName()+(i+1));
        }

        // extra non-text layers
        extraLayers = new ArrayList<LayerInfo>();
        // extra text layers
        textLayers = new ArrayList<LayerInfo>();
        
        // contact maps
        metalContacts = new HashMap<String,List<Element>>();
        otherContacts = new HashMap<String,List<Element>>(); 
        otherTransistors = new HashMap<String,List<Element>>(); 
        multiContacts = new HashMap<String,List<Element>>();
        generalContacts = new HashMap<String,List<Element>>();
        capacitors = new HashMap<String,List<Element>>();
        allArcs = new HashMap<String,Xml.ArcProto>();
        allNodeNames = new HashMap<String,String>();
    }
    ...
    public void addContact(Element con, String lay1, String lay2, boolean isMetal) { addItemToElementMap(con, lay1, lay2, (isMetal) ? metalContacts : multiContacts); }
    
	String getTechName() { return tech_name; }
	public void setTechName(String s) { tech_name = stripQuotes(s); }

	String getTechDescription() { return tech_description; }
	public void setTechDescription(String s) { tech_description = stripQuotes(s); }
	...
	public void setNumMetalLayers(int n)
	{
        ...
        {
        	if (lineReader != null)
        		System.out.println("Setting zero as number of metal layers (line " + lineReader.getLineNumber() + ")");
            num_metal_layers = 0;
            allocateVariables();
        ...
        // n size arrays
        List<WizardField> new_metal_width = new ArrayList<WizardField>(n);
        List<WizardField> new_metal_spacing = new ArrayList<WizardField>(n);
        List<WizardField> new_metal_minarea = new ArrayList<WizardField>(n);
        List<WizardField> new_metal_enclosedarea = new ArrayList<WizardField>(n);
        double [] new_metal_antenna_ratio = new double[n];

        for(int i=0; i<smallest; i++)
        {
            new_metal_width.add(metal_width.get(i));
            new_metal_spacing.add(metal_spacing.get(i));
            new_metal_minarea.add(metal_minarea.get(i));
            new_metal_enclosedarea.add(metal_enclosedarea.get(i));
            new_metal_antenna_ratio[i] = metal_antenna_ratio[i];
        }
		for(int i=smallest; i<n; i++)
        {
            new_metal_width.add(new WizardField());
            new_metal_spacing.add(new WizardField());
            new_metal_minarea.add(new WizardField());
            new_metal_enclosedarea.add(new WizardField());
        }
        ...
        // n-1 size arrays
        List<WizardField> new_via_size = new ArrayList<WizardField>(n-1);
        List<WizardField> new_via_spacing = new ArrayList<WizardField>(n-1);
        List<WizardField> new_via_array_spacing = new ArrayList<WizardField>(n-1);
        List<WizardField> new_via_overhang_inline = new ArrayList<WizardField>(n-1);
		for(int i=0; i<smallest-1; i++)
        {
			new_via_size.add(via_size.get(i));
			new_via_spacing.add(via_inline_spacing.get(i));
			new_via_array_spacing.add(via_array_spacing.get(i));
			new_via_overhang_inline.add(via_overhang.get(i));
        }
		for(int i=smallest-1; i<n-1; i++)
        {
			new_via_size.add(new WizardField());
			new_via_spacing.add(new WizardField());
			new_via_array_spacing.add(new WizardField());
			new_via_overhang_inline.add(new WizardField());
        }
        ...
        {
            new_gds_metal_layer[i] = new LayerInfo(getMetalName()+(i+1));
        }
        ...
        for(int i=smallest-1; i<n-1; i++) new_gds_via_layer[i] = new LayerInfo(getViaName()+(i+1));
    ...
    private boolean isComplexCase() { return withExtraDummyLayers() || withExtraTransistors();} // comp
    private boolean withExtraDummyLayers() { return caseFlag >= 1; } // 1 or 2
    private boolean withExtraTransistors() { return caseFlag > 1; } // 2
    private boolean withDefaultSquareMetals() {return defaultSquareContacts; } 
    private boolean isBasicCase() { return caseFlag == -1;} // basic case = only metals
    public void setCaseFlag(int b) { assert(b > -2 && b < 3); caseFlag = b; }  // -1, 0, 1, 2
    
    private void setShortNameFlag(boolean b) { shortNameFlag = b; }
    private void setDefaultSquareContactsFlag(boolean b) { defaultSquareContacts = b; }
	private String getMetalName() { return (shortNameFlag ? ("M") : ("Metal-")); }
	private String getViaName() { return (shortNameFlag ? ("VIA") : ("Via-")); }
	private String getDName() { return (shortNameFlag ? ("D") : ("DMY-")); } // for dummy layers
	private String getExclName(String s) { return (shortNameFlag ? ("D" + s + "EXCL") : ("DEXCL-"+s)); } // for dummy layers
	...
	// METAL RULES
	WizardField getMetalWidth(int i) { return metal_width.get(i); }
	void setMetalWidth(int met, WizardField value) { metal_width.set(met, value); }
	WizardField getMetalSpacing(int i) { return metal_spacing.get(i); }
	void setMetalSpacing(int met, WizardField value) { metal_spacing.set(met, value); }

	// VIA RULES
	WizardField getViaSize(int i) { return via_size.get(i); }
	void setViaSize(int via, WizardField value) { via_size.set(via, value); }
	WizardField getViaSpacing(int i) { return via_inline_spacing.get(i); }
	void setViaSpacing(int via, WizardField value) { via_inline_spacing.set(via, value); }
	WizardField getViaArraySpacing(int i) { return via_array_spacing.get(i); }
	void setViaArraySpacing(int via, WizardField value) { via_array_spacing.set(via, value); }
	WizardField getViaOverhangInline(int i) { return via_overhang.get(i); }
	void setViaOverhangInline(int via, WizardField value) { via_overhang.set(via, value); }
	...
	private String errorInData()
	{
		...
		for(int i=0; i<num_metal_layers; i++)
			if (metal_width.get(i).value == 0) return "Metal panel: Invalid " + getMetalName() + (i+1) + " width";

		// check the Via data
		for(int i=0; i<num_metal_layers-1; i++)
			if (via_size.get(i).value == 0) return "Via panel: Invalid " + getViaName() + (i+1) + " size";
		return null;
	}
	...
	{
		String fileName = OpenFile.chooseInputFile(FileType.ANY, "Technology Wizard File", null);
		if (fileName == null) return false;
        return importDataFromWizardFormat(fileName);
    }

	/** Auxiliary function to deal with varName and varValue associated to a parameter
	 * @return true on successful parse; false on failure. Values are in lower case.
	 */
	public boolean parseParameter(String buf, boolean semiColonCheck, String values[])
	{
        ...
        {
        	// need to read the rest of the lines until ")" is found
        	valid = readUntilKeyfound(")");
        }
        values[0] = varName.toUpperCase();
        values[1] = stripQuotes(varValue.toUpperCase());
        return valid;
	}
	 ...
	 * 
	 */
	public boolean readUntilKeyfound(String key)
	{
		boolean found = false;
		// Read until the ")" is found
    	try 
    	{
    		while (!found)
    		{
		...
		{
			System.out.println("Error while looking for '" + key + "' (line " + lineReader.getLineNumber() + ")");
		}
    	return found;
	}
    ...
    public boolean importDataFromWizardFormat(String fileName)
    {
        ...
        try
		{
            ...
            for(;;)
			{
                ...
                if (varName.equalsIgnoreCase("tech_name")) setTechName(varValue); else
                ...
                if (varName.equalsIgnoreCase("shortnames")) setShortNameFlag(Boolean.valueOf(varValue)); else // M1 instead of Metal-1, VIA1 instead of Via-1
                ...
                if (varName.toLowerCase().contains("metal_contacts_series")) fillSeries(varValue, metalContacts, 
                		varName.toLowerCase().contains("*")); else
                if (varName.equalsIgnoreCase("contacts_series")) fillSeries(varValue, otherContacts, false); else
                // transistors
                if (varName.equalsIgnoreCase("transistors_series")) fillSeries(varValue, otherTransistors, false); else
                // capacitors
                if (varName.equalsIgnoreCase("capacitors_series")) fillSeries(varValue, capacitors, false); else
                // more generic contacts that are not multicuts.
                if (varName.toLowerCase().contains("multi_contacts_series")) fillSeries(varValue, multiContacts, varName.toLowerCase().contains("*")); else
                ...
                // Special layers
                if (varName.toLowerCase().startsWith("extra_layer")) fillLayerSeries(varValue, extraLayers); else
                ...
                if (varName.equalsIgnoreCase("gds_diff_layer")) diff_layer.setGDSData((varValue)); else
                if (varName.equalsIgnoreCase("gds_poly_layer")) poly_layer.setGDSData((varValue)); else
                if (varName.equalsIgnoreCase("gds_nplus_layer")) nplus_layer.setGDSData((varValue)); else
                if (varName.equalsIgnoreCase("gds_pplus_layer")) pplus_layer.setGDSData((varValue)); else
                if (varName.equalsIgnoreCase("gds_nwell_layer")) nwell_layer.setGDSData((varValue)); else
                if (varName.equalsIgnoreCase("gds_contact_layer")) contact_layer.setGDSData((varValue)); else
                if (varName.equalsIgnoreCase("gds_metal_layer")) metal_layers = setGDSDataArray(varValue, num_metal_layers, getMetalName()); else
                if (varName.equalsIgnoreCase("gds_via_layer")) via_layers = setGDSDataArray(varValue, num_metal_layers - 1, getViaName()); else
                if (varName.equalsIgnoreCase("gds_marking_layer")) marking_layer.setGDSData((varValue));
                else
                {
                    Object obj = null;
                    if (single)
            ...
            if (!validFile)
            {
               Job.getUserInterface().showErrorMessage("'" + fileName + 
            		   "' is not a valid Parameters file for the Technology Creation Wizard.\n" + 
    ...
    {
    	return findWizardField(varName, true);
    }
    
    private WizardField findWizardField(String varName, boolean createOne)
    {
        ...
        if (!createOne) return null; // just not found
	...
	private LayerInfo [] setGDSDataArray(String str, int len, String extra)
	{
        ...
        while (parse.hasMoreTokens())
        {
            ...
            else
            {
                ...
                if (!value.equals(","))
                    foundArray[count++].setGDSData(value);
            }
        }
        return foundArray;
	}
	...
	{
		List<WizardField> foundArray = new ArrayList<WizardField>(num_metal_layers);
		for(int i=0; i<num_metal_layers; i++) foundArray.add(new WizardField());
		fillWizardArray(str, foundArray, false);
		double [] retArray = new double[foundArray.size()];
		for(int i=0; i<foundArray.size(); i++)
			retArray[i] = foundArray.get(i).value;
		return retArray;
	}
    ...
    private void fillWizardArray(String str, List<WizardField> fieldArray, boolean getRule)
	{
    	int lineNumber = lineReader.getLineNumber();
		if (!str.startsWith("("))
		{
			Job.getUserInterface().showErrorMessage("Array does not start with '(' on '" 
					+ str + "', line:" + lineNumber, "Syntax Error In Technology File");
			return;
		}
        ...
        while (parse.hasMoreTokens())
        {
            ...
            {
            	Job.getUserInterface().showErrorMessage("Out of bound index in array while parsing '" 
            			+ str  + "', line:" + lineNumber, "Syntax Error In Technology File");
            	return;
            }
            	
            if (getRule) // assume "<name>" format
            {
                fieldArray.get(index++).rule = value;
            }
            else
            {
                double v = TextUtils.atof(value);
                fieldArray.get(index++).value = v;
            }
        }
//        int pos = 1;
//		for(;;)
...
//		}
        ...
        while (parse.hasMoreTokens())
        {
            ...
            while (p.hasMoreTokens())
            {
                ...
                switch (itemCount)
                {
                    ...
                    case 2:
                    case 3:
                        layer.setLayerInformation(s, lineReader);
                        break;
                    default:
                        assert(false);
                }
    ...
    private void fillRulesSeries(String str, List<WizardField> rules)
    {
        int index = str.indexOf("(");
        ...
        while (parse.hasMoreTokens())
        {
            ...
            while (p.hasMoreTokens())
            {
                ...
                if (pos >= rules.size())
                    assert(pos < rules.size()); // no out of bound array
                ...
                while (x.hasMoreTokens() && itemCount < 2)
                {
                    ...
                    {
                        case 0: // rule value
                           rules.get(pos).value = Double.valueOf(item);
                            break;
                        case 1: // rule name
                            rules.get(pos++).rule = item;
                            break;
                    }
                    itemCount++;
                }
    ...
    private void fillSeries(String str, Map<String,List<Element>> elementMap, boolean sizeByCutLayer)
    {
        StringTokenizer parse = new StringTokenizer(str, "[]", false);
            ...
            switch (steps)
            {
	            ...
	            case 1: // Layer names information
	                ...
	                while (p.hasMoreTokens())
	                {
	                    ...
	                    {
	                    	System.out.println("Error in Technology Creation Wizard: no matching number of layers in fillSeries in line " + lineReader.getLineNumber());
	                    	return;
	                    }
	                    if (layerNames.size() < 2)
	                    {
	                    	System.out.println("Error in Technology Creation Wizard: at least two layers are needed in fillSeries in line " + lineReader.getLineNumber());
	                    	return;
	                    }
	                    
	                    elem = new Element(prefix, sizeByCutLayer);
	                    for (int i = 0; i < layerNames.size(); i++)
	                    {
	                        ...
	                        if (tmp instanceof NotCenterRectLayerNode)
	                        {
	                        	...
	                        	node = new NotCenterRectLayerNode(name,
	                                tmp.valueX.value, tmp1.valueXR, tmp.valueX.rule,
	                                tmp.valueY.value, tmp1.valueYT, tmp.valueY.rule);
	                        }
	                        else 
	                        {
	                        	if (tmp.inline != null)
	                        	{
		                        	node = new RectLayerNode(name,
		                            tmp.valueX.value, tmp.valueX.rule,
		                            tmp.valueY.value, tmp.valueY.rule, tmp.inline.value, tmp.inline.rule);
	                        	}
	                        	else
	                        	{
		                        	node = new RectLayerNode(name,
	                    ...
	                    addItemToElementMap(elem, layer1, layer2, elementMap);
	                }
	            	break;
	            ...
	            case 0: // rules for layers
	                ...
	                while (p.hasMoreTokens())
	                {
	                	...
	                	if (s.startsWith("{"))
	                		parseNotCenteredNode(s, nodeList);
	                	else
	                		parseNode(s, nodeList);
	                }
	            	break;
	            default:
	...
	private boolean parseNode(String s, List<RectLayerNode> nodeList)
	{
        // layer info
        //int itemCount = 0; // 4 max items: metal layer, overhang X, overhang X rule,
		// overhang Y, overhang Y rule, inline space, inline spacing rule
    	String key = ", ";
    	assert(!s.startsWith("{")); // only one list
    	
        StringTokenizer x = new StringTokenizer(s, key, false);
        double overX = 0, overY = 0, inlineSpace = 0;
        String overXS = null, overYS = null, inlineS = null;
        List<String> elems = new ArrayList<String>();
        
        while (x.hasMoreTokens())
            elems.add(x.nextToken());
        
        // In case of nonmulti contacts, the 6 items are overX, overXS, overY, overYS, inlineSpacing, inlineSpacingS
    	int itemCount = 0;
            ...
            try
            {
            	switch (itemCount)
            	{
                	...
                	case 3: // overhang Y rule name
            			overYS = item;
                		break;
                	case 4: // inline spacing
                		inlineSpace = Double.valueOf(item);
                		break;
                	case 5: // inline spacing rule name
                		inlineS = item;
                		break;
        		}
            } catch (Exception e)
            {
            	System.out.println("Invalid syntax in '" + s + "' while reading fillSeries in line " + lineReader.getLineNumber());
            	return false;
            }
            itemCount++;
    	}
    	if (itemCount == 6) // includes inline spacing
    		nodeList.add(new RectLayerNode("", overX, overXS, overY, overYS, inlineSpace, inlineS));
    	else
    		nodeList.add(new RectLayerNode("", overX, overXS, overY, overYS));
    	return true;
    }
    
    private boolean parseNotCenteredNode(String s, List<RectLayerNode> nodeList)
    {
      	String key = ", {}";
    	int maxVariables = 6;
    	assert(s.startsWith("{"));
        ...
        assert(elems.size() % maxVariables == 0); // almost factor of maxVariables
        
        // In case of transistors, elements with 6 items reflect left, right values for X and Y
        // In case of nonmulti contacts, the 6 items are overX, overXS, overY, overYS, inlineSpacing, inlineSpacingS
        for (int j = 0; j < numLists; j++)
        {
        	...
        	{
        		String item = elems.get(j*maxVariables + i);
                try
                {
                	switch (itemCount)
                	{
                    	case 0: // overhang X left value
                    		overX = Double.valueOf(item);
                    		break;
                    	case 1: // overhang X right value
                    	...
                    	case 2: // overhang X rule name
                    		...
                    		overXS = item;
                    		break;
                    	case 3: // overhang Y bottom value
                    		overY = Double.valueOf(item);
                    		break;
                    	case 4: // overhang Y top value
                    		overTop = Double.valueOf(item);
                    		break;
                    	case 5: // overhang Y rule name
                    		overYS = item;
                    		break;
            		}
                } catch (Exception e)
            ...
            nodeList.add(new NotCenterRectLayerNode("", overX, overRight, overXS, overY, overTop, overYS));
        }
            ...
            while (par.hasMoreTokens())
            {
                ...
                if (count%2 == 1) // odd
                {
                    ...
                    while (p.hasMoreTokens())
                    {
                        ...
                        assert(index == 2);
                        EPoint point = EPoint.fromLambda(points[0], points[1]);
                        pList.add(point);
                    }
                }
...
//        generalContacts.add(con);
	...
	private void dumpNumbers(PrintWriter pw)
	{
		...
		{
			if (i > 0) pw.print(", ");
			pw.print(TextUtils.formatDouble(metal_width.get(i).value));
		}
		pw.println(");");
		...
		{
			if (i > 0) pw.print(", ");
			pw.print("\"" + metal_width.get(i).rule + "\"");
		}
		pw.println(");");
		...
		{
			if (i > 0) pw.print(", ");
			pw.print(TextUtils.formatDouble(metal_spacing.get(i).value));
		}
		pw.println(");");
		...
		{
			if (i > 0) pw.print(", ");
			pw.print("\"" + metal_spacing.get(i).rule + "\"");
		}
		pw.println(");");
		pw.println();
		...
		{
			if (i > 0) pw.print(", ");
			pw.print(TextUtils.formatDouble(via_size.get(i).value));
		}
		pw.println(");");
		...
		{
			if (i > 0) pw.print(", ");
			pw.print(TextUtils.formatDouble(via_inline_spacing.get(i).value));
		}
		pw.println(");");
		...
		{
			if (i > 0) pw.print(", ");
			pw.print("\"" + via_inline_spacing.get(i).rule + "\"");
		}
		pw.println(");");
		pw.println();
		...
		{
			if (i > 0) pw.print(", ");
			pw.print(TextUtils.formatDouble(via_array_spacing.get(i).value));
		}
		pw.println(");");
		...
		{
			if (i > 0) pw.print(", ");
			pw.print("\"" + via_array_spacing.get(i).rule + "\"");
		}
		pw.println(");");
		...
		{
			if (i > 0) pw.print(", ");
			pw.print(TextUtils.formatDouble(via_overhang.get(i).value));
		}
		pw.println(");");
		...
		{
			if (i > 0) pw.print(", ");
			pw.print("\"" + via_overhang.get(i).rule + "\"");
		}
		pw.println(");");
		pw.println();
	 ...
	 * @param checkErrorsInData true if data must be checked for invariants.
	 */
	public void writeXML(boolean checkErrorsInData)
	{
		if (checkErrorsInData)
		{
			// Only valid for the TecheEditorWizard
...
//            }
    ...
    {
        return makeXmlMulticut(0, 0, 0, 0, lb, sizeRuleX, sizeRuleY, sepRule, sepRule2D);
    }
    ...
    {
        return makeXmlMulticut(lx, -1, hx, 1, ly, -1, hy, 1, lb, sizeRuleX, sizeRuleY, sepRule, sepRule2D);
    }
...
//        double longD = DBMath.isGreaterThan(extLayer1, extLayer2) ? extLayer1 : extLayer2;
        ...
        return (makeXmlPrimitiveCon(nodeGroups, composeName, PrimitiveNode.Function.CONTACT, -1, -1,
            null, /*new SizeOffset(longD, longD, longD, longD),*/ portNames,
                ...
                makeXmlMulticut(conLayer, contSize, contSize, spacing, arraySpacing))); // contact
    }
    ...
    /**
     * Special treatment for extra non-text and text layers
     */
    private void generateExtraLayers(Xml.Technology t, List<LayerInfo> list, 
    		List<PaletteGroup> paletteList, Map<Xml.Layer,WizardField> layerMap, boolean addPin)
    {
        ...
        for (LayerInfo info : list)
        {
            ...
            else if (info.graphicsColor != null)
            {
                ...
                graph = new EGraphics(displayPatterned, displayPatterned, info.graphicsOutline, 0,
                    ...
                    1, true, (info.graphicsPattern != null) ? info.graphicsPattern : nullPattern);
            }
            if (graph == null)
                graph = new EGraphics(false, false, null, 0, 255, 0, 0, 0.4, true, nullPattern);

            boolean actualAddPin = addPin && info.addArc;
            boolean actualPureNode = info.functionExtras != Layer.Function.NONELEC; // info.addArc
            if (actualAddPin && DBMath.areEquals(info.width, 0))
            ...
            portNames.clear();
            portNames.add(info.name);
            
            Xml.Layer layer = makeXmlLayer(t.layers, layerMap, info.name, info.function, info.functionExtras, graph,
                wf, actualPureNode, info.addArc, info.name);
            ...
            {
                // 
                if (info.function == Layer.Function.DIFF)
                {
                	layer.pureLayerNode.portArcs.add("N-Diff");
                	layer.pureLayerNode.portArcs.add("P-Diff");
                }
                
                info.grp = new PaletteGroup();
                // search for the arc function by name
                ArcProto.Function fun = ArcProto.Function.findByPrintName(info.function.toString());
                if (fun == null)
                {
                	if (Job.getDebug())
                		System.out.println("It didn't find equivalent arc function for layer '" + layer.name + ", function='" + info.function + "'. Assigining unknown function");
                	fun = ArcProto.Function.UNKNOWN;
                }
                info.grp.addArc(makeXmlArc(t, info.name, fun, 0, makeXmlArcLayer(layer, wf)));
...
//        t.scaleRelevant = isScaleRelevant();
        ...
        List<Xml.Layer> dummyViaLayers = new ArrayList<Xml.Layer>();
        ...
        for (int i = 0; i < num_metal_layers; i++)
        {
            ...
            switch (metLayDig)
            {
                ...
                case 2: tcol = 5;   break;
                default:
                	pattern = getPatternByCode(metLayDig);
            }
            ...
            if (withExtraDummyLayers())
            {
                ...
                layer = makeXmlLayer(t.layers, getDName()+metalName, Layer.Function.getDummyMetal(i), 0, graph,
                    5*metal_width.get(i).value, true, false);
                ...
                layer = makeXmlLayer(t.layers, getExclName(metalName), Layer.Function.getDummyExclMetal(i), 0, graph,
                    2*metal_width.get(i).value, true, false);
        ...
        for (int i = 0; i < num_metal_layers - 1; i++)
        {
            ...
            viaLayers.add(makeXmlLayer(t.layers, layerMap, viaName, fun, Layer.Function.CONMETAL,
                graph, via_size.get(i), true, false));
            
            if (withExtraDummyLayers())
            {
                // dummy layers
                graph = new EGraphics(true, true, null, 0, r, g, b, opacity, false, nullPattern);
                // is getDummyMetal() the right function?
                Xml.Layer layer = makeXmlLayer(t.layers, getDName()+viaName, Layer.Function.getDummyMetal(i), 0, graph,
                    via_size.get(i).value, true, false);
        ...
        generateExtraLayers(t, extraLayers, extraPaletteList, layerMap, true); 
        
        // Extra text layers
        List<PaletteGroup> textPaletteList = new ArrayList<PaletteGroup>();
        generateExtraLayers(t, textLayers, textPaletteList, layerMap, false); 
        ...
        for(int i=1; i<=num_metal_layers; i++)
        {
            ...
            group.addArc(makeXmlArc(t, getMetalName()+i, ArcProto.Function.getContact(i), ant,
                makeXmlArcLayer(metalLayers.get(i-1), metal_width.get(i-1))));
        }
        
        List<PaletteGroup> polysGroup = new ArrayList<PaletteGroup>();
        if (!isBasicCase())
            addStandardElements(t, layerMap, metalLayers, allGroups, polysGroup);
        ...
        for (int i = 0; i < num_metal_layers; i++)
        {
            double hla = scaledValue(metal_width.get(i).value / 2);
        ...
        if (withDefaultSquareMetals())
        {
	        for(int i=1; i<num_metal_layers; i++)
			{
                ...
                double viaSize = scaledValue(via_size.get(i-1).value);
                double viaSpacing = scaledValue(via_inline_spacing.get(i-1).value);
                double viaArraySpacing = scaledValue(via_array_spacing.get(i-1).value);
                String name = lb.name + "-" + lt.name;

                double longDist = scaledValue(via_overhang.get(i-1).value);
        ...
        // metal contacts
        for (Map.Entry<String,List<Element>> e : metalContacts.entrySet())
        {
            // generic contacts
            for (Element c : e.getValue())
            {
                ...
                int via = (j>i)?i:j;
                double metalContSize = scaledValue(via_size.get(via-1).value);
                double spacing = scaledValue(via_inline_spacing.get(via-1).value);
                double arraySpacing = scaledValue(via_array_spacing.get(via-1).value);
                Xml.Layer metalConLayer = viaLayers.get(via-1);
                double viaHalfSize = via_size.get(via-1).value / 2;
                double h1x = scaledValue(viaHalfSize + verticalLayer.valueX.value);
                double h1y = scaledValue(viaHalfSize + verticalLayer.valueY.value);
                double h2x = scaledValue(viaHalfSize + horizontalLayer.valueX.value);
                double h2y = scaledValue(viaHalfSize + horizontalLayer.valueY.value);
                ...
                double maxX = scaledValue(Math.max(verticalLayer.valueX.value, horizontalLayer.valueX.value));
                double maxY = scaledValue(Math.max(verticalLayer.valueY.value, horizontalLayer.valueY.value));
                SizeOffset so = (c.sizeByCutLayer) ? 
                		new SizeOffset(maxX, maxX, maxY, maxY) : 
                ...
                metalPalette[via-1].addElement(makeXmlPrimitiveCon(t.nodeGroups, p + name, PrimitiveNode.Function.CONTACT, -1, -1,
                    ...
                    makeXmlMulticut(metalConLayer, metalContSize, metalContSize, spacing, arraySpacing)), c.prefix); // contact
            }
        }
        
        // Generic contacts which might be based on extraLayers
        addGenericContacts(t, multiContacts, extraPaletteList, polysGroup);
        ...
        // Extra text layers with pins/arcs
        //allGroups.addAll(textPaletteList);
        ...
        for (int i = 0; i < num_metal_layers; i++) {
            ...
            {
                // Type is always 1
                makeLayerGDS(t, dummyMetalLayers.get(i), metal_layers[i].getLayerNumber(GDSLayerType.DRAWING) + "/1");
            ...
            if (withExtraDummyLayers())
                // Type is always 1
            	makeLayerGDS(t, dummyViaLayers.get(i), String.valueOf(via_layers[i].getLayerNumber(GDSLayerType.DRAWING)) + "/1");
        }
        ...
        for (int i = 0; i < num_metal_layers; i++) {
            Xml.Layer met = metalLayers.get(i);
            makeLayerRuleMinRule(t, met, DRCTemplate.DRCRuleType.MINWID, metal_width.get(i));
            makeLayerRuleMinRule(t, met, DRCTemplate.DRCRuleType.MINAREA, metal_minarea.get(i));
            makeLayerRuleMinRule(t, met, DRCTemplate.DRCRuleType.MINENCLOSEDAREA, metal_enclosedarea.get(i));
            makeLayersRule(t, met, DRCTemplate.DRCRuleType.SPACING, metal_spacing.get(i).rule, metal_spacing.get(i).value);

            if (i >= num_metal_layers - 1) continue;
            Xml.Layer via = viaLayers.get(i);
            makeLayerRuleMinRule(t, via, DRCTemplate.DRCRuleType.MINWID, via_size.get(i));
            makeLayersRule(t, via, DRCTemplate.DRCRuleType.SPACING, via_inline_spacing.get(i).rule, via_inline_spacing.get(i).value);
        }
        // wide metal rules
        ...
        // checking the best number of columns
        
        // default case
        int val = t.menuPalette.menuBoxes.size() / 60;
        int rest = t.menuPalette.menuBoxes.size() % 60; // 60 seems a good number of cells (3 columns of 20)
        if (rest != 0)
        	val++; // add extra section of 3 columns

        WizardField wf = findWizardField("numberOfPaletteColumns", false); /// check first if the # of columns is predefined
        if (wf != null)
        	val = (int)wf.value;
        else
        	val = 3 * val; // 3 as original default
        t.menuPalette.numColumns = val; // 3;
...
//    }
    ...
    private void createAnalogElements(Xml.Technology t, List<Xml.Layer> metalLayers, List<PaletteGroup> polysGroup)
    {
        ...
        // Analog Capacitors
        /*************************************/
        for (Map.Entry<String,List<Element>> e : capacitors.entrySet())
        {
...
//            polyNoScaled = 2 * (contact_poly_overhang.value) + contact_size.value;
    ...
    private void addMetalElements(Xml.Technology t, Xml.Layer conLayer, double spacing, WizardField width, WizardField overhang,
                                  List<Xml.NodeLayer> nodesList, List<Xml.PrimitivePort> nodePorts)
    {
        Xml.Layer m1Layer = t.findLayer(getMetalName()+"1");
        List<String> portNames = new ArrayList<String>();
        ...
        nodesList.add(makeXmlMulticut(cutEnd+contSize, -1, -cutEnd, -1, cutEndY, -1, cutEndY, 1,
            conLayer, contSize, contSize, spacing, spacing));
        // right
    ...
    /**
     * To add standard diff/well/poly contacts plus transistors
     * @param t
     * @param layerMap
     * @param metalLayers
     * @param allGroups
     * @param polysGroup Needed as parameter so nomulticontacts might add other elements to this group
     */
                                     ...
                                     List<PaletteGroup> polysGroup)
    {
        ...
        // only for standard cases when getExtraTransistors or dummy layers is false
        if (!isComplexCase())
        {
            if (POLY_CONTACT_SAME_AS_DIFF_CONTACT) {
                ...
                if (via_overhang.size()> 0)
                    polyGroup.addElement(makeContactSeries(t.nodeGroups, polyLayer.name, contSize, polyConLayer, contSpacing, contArraySpacing,
                            scaledValue(contact_poly_overhang.value), polyLayer,
                            scaledValue(via_overhang.get(0).value), m1Layer), null);
                else
        ...
        for (Map.Entry<String,List<Element>> e : otherContacts.entrySet())
        {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            diffG.addElement(makeXmlPrimitiveCon(t.nodeGroups, "F-"+composeName, PrimitiveNode.Function.CONTACT,
                ...
                wellNode, // well layer
                makeXmlMulticut(diffConLayer, contSize, contSize, contSpacing, contArraySpacing)), "Full-" + diffNames[i]); // contact
        }
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            g.addElement(makeXmlPrimitiveCon(t.nodeGroups, "F-"+composeName, func,
                ...
                makeXmlMulticut(diffConLayer, contSize, contSize, contSpacing, contArraySpacing)), "Full-"+diffNames[i] + "W"); // contact
        }
...
//                double absVal = (contact_poly_overhang.v - via_overhang[0].v);
                ...
                nodesList.add(makeXmlMulticut(cutEnd2, -1, -cutDistance, -1, cutSizeHalf, -1, cutSizeHalf, 1,
                    polyConLayer, contSize, contSize, contArraySpacing, contArraySpacing));
                ...
                nodesList.add(makeXmlMulticut(cutEnd2, -1, -cutEnd, -1, cutEndY, -1, cutEndY, 1,
                    diffConLayer, contSize, contSize, contArraySpacing, contArraySpacing));
                // right
        ...
        // more generic and easy to create transistors
        for (Map.Entry<String,List<Element>> e : otherTransistors.entrySet())
        {
        	...
        	for (Element el : elemList)
        	{
                ...
                Xml.PrimitivePort leftP = null, rightP = null, topP = null, bottomP = null;
                List<Xml.PrimitivePort> otherPorts = new ArrayList<Xml.PrimitivePort>();
                boolean foundDiff = false;
                double diffWidth = 0;
                int groupPalette = -1;
                
                for (LayerNode n : el.layers)
                {
                	RectLayerNode rn = (RectLayerNode)n;
                	...
                	if (rn instanceof NotCenterRectLayerNode)
                	{
                		...
                		double absB = Math.abs(bottom);
                		double absT = Math.abs(top);
                		int factorB = -1, factorT = 1;
                		if (bottom < 0 && top < 0) {factorT= -1; bottom = absB;}
                		else if (top > 0 && bottom > 0){factorB = 1; bottom = -absB;}
                		
	                	nodesList.add(makeXmlNodeLayer(left, -1, right, 1, 
	                			bottom, factorB, top, factorT,
	                			xmlLayer, Poly.Type.FILLED, true, false, -1));
                	...
                	else
                	{
	                	...
	                	double w = rn.valueX.value/2;
	                	double h = rn.valueY.value/2;
	                	...
	                	if (isPolyLayer)
	                	{
		                	// 3 electric nodeLayers. Must have found diff alreay
	                		assert(foundDiff);
	                		// left
		                	nodesList.add(makeXmlNodeLayer(w, -1, -diffWidth, -1, h, -1, h, 1,
		                			xmlLayer, Poly.Type.FILLED, true, true, 0));
		                	// right
		                	nodesList.add(makeXmlNodeLayer(-diffWidth, 1, w, 1, h, -1, h, 1,
		                			xmlLayer, Poly.Type.FILLED, true, true, 2));
		                	// gate
		                	nodesList.add(makeXmlNodeLayer(diffWidth, -1, diffWidth, 1, h, -1, h, 1,
		                			t.findLayer("PolyGate"), Poly.Type.FILLED, true, true, -1));
	                	}
	                	else if (isDiffLayer)
	                	{
	                		foundDiff = true;
	                		diffWidth = Math.abs(w);
		                	// two electric nodeLayers
	                		// bottom
		                	nodesList.add(makeXmlNodeLayer(w, -1, w, 1, h, -1, 0, 1,
		                			xmlLayer, Poly.Type.FILLED, true, true, 3));
		                	// top
		                	nodesList.add(makeXmlNodeLayer(w, -1, w, 1, 0, -1, h, 1,
		                			xmlLayer, Poly.Type.FILLED, true, true, 1));
		                	
		                	groupPalette = xmlLayer.function == (com.sun.electric.technology.Layer.Function.DIFFP) ? 0 : 1;
	                	}

	                	// not electrical nodeLayer
	                	nodesList.add(makeXmlNodeLayer(w, -1, w, 1, h, -1, h, 1,
	                			xmlLayer, Poly.Type.FILLED, true, false, -1));

	                	
	                	// look if it has already an arc associated
	                	if (t.findArc(rn.layer) == null) continue; // layer doesn't need port
	
	                	
	                    portNames.clear();
	                    portNames.add(rn.layer);
                		
	                	// trying to determine the center
	                	if (isPolyLayer)
	                	{
	                		soy = h;
	                		double absW = Math.abs(w);
	                		leftP = (makeXmlPrimitivePort(rn.layer+"-left", 180, 90, 0, minFullSize,
	                				-absW, -1, -absW, -1, 0, -1, 0, 1, portNames));
	                		rightP = (makeXmlPrimitivePort(rn.layer+"-right", 0, 180, 0, minFullSize,
	                				absW, 1, absW, 1, 0, -1, 0, 1, portNames));
	                	}
	                	else if (isDiffLayer)
	                	{
	                		// there could be multiple diff layers but getting the largest should do the trick.
	                		if (w > sox) 
	                			sox = w;
	                		double absH = Math.abs(h/2);
	                		topP = (makeXmlPrimitivePort(rn.layer+"-top", 90, 90, 1, minFullSize,
	                				0, -1, 0, 1, absH, 1, absH, 1, portNames));
	                		bottomP = (makeXmlPrimitivePort(rn.layer+"-bottom", 270, 90, 2, minFullSize,
	                				0, -1, 0, 1, -absH, -1, -absH, -1, portNames));
	                	}
                	}
                }
                ...
                nodePorts.add(leftP); nodePorts.add(topP); nodePorts.add(rightP); nodePorts.add(bottomP);
                nodePorts.addAll(otherPorts);
                
                String name = "G" + el.prefix;
                PrimitiveNode.Function f = (el.prefix.contains("N")) ? PrimitiveNode.Function.TRANMOS : PrimitiveNode.Function.TRAPMOS;
                sox = maxX - sox;
                soy = maxY - soy;
                Xml.PrimitiveNodeGroup n = makeXmlPrimitive(t.nodeGroups, name + "-Transistor", f, 0, 0, 0, 0,
                        new SizeOffset(sox, sox, soy, soy), nodesList, nodePorts, null, false);
                transPalette[groupPalette].addElement(n, name);
        	}
        }
        ...
        //allGroups.addAll(polysGroup); // has to be in main function to add possible poly contacts from nomulti
    ...
    private void addContactsOrCapacitors(Xml.Technology t, List<Element> contacts, List<Xml.Layer> metalLayers,
                                         ...
                                         PaletteGroup polyGroup, boolean capacitor)
    {
        ...
        for (Element c : contacts)
        {
            ...
            // cut
            nodes[count++] = makeXmlMulticut(conLay, contSize, contSize, contSpacing, contArraySpacing);
            // metal
...
//                if (!pSubstrateProcess || otherLayerPort == pwellLayer)
     ...
     ***************************************************************************************************/
    private void addGenericContacts(Xml.Technology t, Map<String,List<Element>> contacts, 
    		List<PaletteGroup> extraPaletteList, List<PaletteGroup> polysGroup)
    {
        ...
        for (Map.Entry<String,List<Element>> e : contacts.entrySet())
        {
            ...
            for (Element c : e.getValue())
            {
                ...
                RectLayerNode cutLayer = (RectLayerNode)c.layers.get(c.layers.size()-1);
                ...
                for (LayerNode l : c.layers)
                {
                    ...
                    if (grp != null) break; // found
                    // Looking if it belongs to existing non-MxMy contact groups like poly
                    if (n.layer.equals(polyLayer.name)) // found Poly
                    {
                    	polyGroup = true;
                    	// doesn't stop here in case, there is a match with a layer in extraLayers
                    }
                }
                if (grp == null)
                {
                	if (polyGroup)
                		grp = polysGroup.get(0); //add it to the first and probably only group in the list
                	else
                	{
                ...
                // Cut info
                Xml.NodeLayer cutNode = null;
                Xml.Layer metalConLayer = t.findLayer(cutLayer.layer);
                
                if (cutLayer.inline != null)
                {
                    double metalContSizeX = scaledValue(cutLayer.valueX.value);
                    double metalContSizeY = scaledValue(cutLayer.valueY.value);
                	double inlineVal = scaledValue(cutLayer.inline.value);
                	cutNode = makeXmlMulticut(metalConLayer, metalContSizeX, metalContSizeY, inlineVal, inlineVal);
                }
                else
                {
                  ...
                  double metalContSizeY = scaledValue(cutLayer.valueY.value/2);
                	cutNode = makeXmlNodeLayer(metalContSizeX, metalContSizeX, metalContSizeY, metalContSizeY,
                            metalConLayer, Poly.Type.FILLED);
                }

                String name = ""; //la.name + "-" + lb.name;
                double maxXL = Double.NEGATIVE_INFINITY, maxYB = Double.NEGATIVE_INFINITY;
                double maxXR = Double.NEGATIVE_INFINITY, maxYT = Double.NEGATIVE_INFINITY;
                double maxX = Double.NEGATIVE_INFINITY, maxY = Double.NEGATIVE_INFINITY;
                double minX = Double.POSITIVE_INFINITY, minY = Double.POSITIVE_INFINITY;
                // the first size-1 are non-cut layers
//                List <Xml.NodeLayer> list = new ArrayList<Xml.NodeLayer>(c.layers.size());
                ...
                {
                	RectLayerNode aL = (RectLayerNode)c.layers.get(i);
                    Xml.Layer la = t.findLayer(aL.layer);
                    assert(la != null);
                    double xl = aL.valueX.value, xr = aL.valueX.value;
                    double yb = aL.valueY.value, yt = aL.valueY.value;

                    // correct the top and right values
                    if (aL instanceof NotCenterRectLayerNode)
                    {
                    ...
                    double h1xr = scaledValue(cutLayer.valueX.value /2 + xr);
                    double h1yb = scaledValue(cutLayer.valueY.value /2 + yb);
                    double h1yt = scaledValue(cutLayer.valueY.value /2 + yt);
                    
                    name += la.name;
                    if (i < (c.layers.size()-2))
                    	name += "-";
                    array[i] = (makeXmlNodeLayer(h1xl, h1xr, h1yb, h1yt, la, Poly.Type.FILLED)); // layer i
                    if (maxX < xr)
                    	maxX = xr;
                    if (maxXR < xr)
                    	maxXR = xr;
                 ...
                 // check if the corresponding arcs exist
                    if (allArcs.get(la.name) == null)
                    	System.out.println("Error: Arc '" + la.name + "' does not exist");
                }
                // adding cut at the end
                array[c.layers.size()-1] = cutNode;

                // max and min to calcute the SizeOffset
                double longX = scaledValue(Math.abs(maxX - minX));
                double longY = scaledValue(Math.abs(maxY - minY));
                maxX = scaledValue(Math.abs(maxX));
                maxY = scaledValue(Math.abs(maxY));
//                if (!foundIrregular)
//                	assert(!foundIrregular && (maxXL == maxXR) && maxXL == maxX);
//                if (!foundIrregular)
//                	assert(!foundIrregular && (maxYB == maxYT) && maxYB == maxY);
                SizeOffset so = (c.sizeByCutLayer) ? 
                		new SizeOffset(maxX, maxX, maxY, maxY) : 
                		new SizeOffset(longX, longX, longY, longY);
