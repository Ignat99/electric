...
public class ToolBar extends JToolBar
{
	...
	public static void setToolbarButtons(EToolBarButton[] buttons)
	{
		...
		User.toolbarOrderPref.setString(sb.toString());
	...
	public static Map<String,String> getCommandToIconMap()
	{
		if (commandToIconMap == null)
		{
			...
			String fileMap = User.toolbarFilesPref.getString();
	...
	public static void setCommandToIconMap(Map<String,String> newMap)
	{
		...
		User.toolbarFilesPref.setString(sb.toString());
	}
	...
	{
		String prefOrder = User.toolbarOrderPref.getString();
		...
		if (prefOrder.length() > 0)
		{
			...
			User.toolbarOrderPref.setString("");
	...
	{
		EDimension val = User.getAlignmentToGrid();
		String valStr = " " + TextUtils.formatDistance(val.getWidth());
		if (val.getWidth() != val.getHeight())
			valStr += "/" + TextUtils.formatDistance(val.getHeight());
		valStr += " ";
		if (TopLevel.isMDIMode())
		{
...
//		/** Describes wiring mode (creating arcs). */			WIRE("Toggle Wiring"),
		...
		/** Describes Routing mode. */							ROUTING;
	}
	...
	{
		if (cm == CursorMode.CLICKZOOMWIRE && User.isRoutingMode()) cm = CursorMode.ROUTING;
		switch (cm)
		{
			case CLICKZOOMWIRE:
			...
			case OUTLINE:
                ...
                Highlight highlight = highlighter.getOneHighlight();
                ElectricObject eObj = highlight.getElectricObject();
                if (eObj != null && eObj instanceof PortInst) eObj = ((PortInst)eObj).getNodeInst();
                if (ni != eObj)
                	System.out.println("WARNING: Selected object is " + ni.describe(false) + " but alternate selection shows " + highlight.getElectricObject());
//                assert ni == eObj;
				...
				if (WindowFrame.getListener() != OutlineListener.theOne)
					OutlineListener.theOne.setNode(highlight, ni);
...
//						high.setPoint(-1);
		...
		KeyStroke.getKeyStroke('H', 0), "ButtonGridFiner", "Edit:Modes:Movement")
    {
        ...
        public boolean isEnabled()
        {
        	EDimension[] vals = User.getAlignmentToGridVector();
            int index = User.getAlignmentToGridIndex();
	...
	public static final EToolBarButton undoCommand = new EToolBarButton("_Undo", 'Z', "ButtonUndo", "Edit")
	{
		public void run()
		{
			...
			} else
			{
				...
				// warn if there are change jobs running
				JobTree jt = (JobTree)JobTree.getExplorerTree();
				List<Job.Inform> jobs = jt.getChangeJobs();
				if (jobs.size() > 0)
				{
					String jobList = "";
                    String [] options = {"Cancel", "Undo"};
                    for(Job.Inform ji : jobs)
                    {
                    	if (jobList.length() > 0) jobList += ", ";
                    	jobList += ji.toString();
                    }
                    int ret = JOptionPane.showOptionDialog(TopLevel.getCurrentJFrame(),
						"Change jobs are still running: " + jobList + ". Really Undo?", "Undo during a Change",
...
//                }
	...
	/**
	 * Update associated ToolBarButtons on all toolbars and updateable menu items on all menubars
	 */
