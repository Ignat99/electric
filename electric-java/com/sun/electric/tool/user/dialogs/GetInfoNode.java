...
public class GetInfoNode extends EModelessDialog implements HighlightListener, DatabaseChangeListener
{
	...
	private List<ArcInst> portArcs;
	private List<PortInst> portObjects;
	...
	private GetInfoNode(Frame parent)
	{
		...
		portArcs = new ArrayList<ArcInst>();
		portObjects = new ArrayList<PortInst>();
	...
	private void doLoadInfo()
	{
		...
		if (wnd != null) {
			...
			{
				if (!h.isValid() || !h.isHighlightEOBJ()) continue;
...
//			if (pnp.findMulticut() != null) multiCutNode = true;
	...
	private void showProperList(boolean showAll)
	{
		...
		if (ports.isSelected())
		{
			...
			for(Iterator<PortInst> it = shownNode.getPortInsts(); it.hasNext(); )
			{
				...
				{
					Variable var = ((Export)pp).getVar(Export.EXPORT_PREFERRED_ARCS);
					if (var != null)
					{
				...
				for(Iterator<Connection> aIt = shownNode.getConnections(); aIt.hasNext(); )
				{
					...
					description = "  Connected at (" + TextUtils.formatDistance(con.getLocation().getX()) + "," +
						TextUtils.formatDistance(con.getLocation().getY()) + ") to " + ai;
					portMessages.add(description);
					portArcs.add(ai);
					portObjects.add(null);
				}
		...
		public boolean doIt() throws JobException
		{
			...
			if (extrafields.scalableTrans)
			{
				if (currentPopupIndex != initialPopupIndex)
				{
					...
					ni.newVar(Technology.TRANS_CONTACT, contactInfo, ep);
				}
			...
			if (extrafields.multiCutNode)
			{
				...
				if (currentPopupIndex != initialPopupIndex)
				{
                    if (currentPopupIndex != NodeLayer.MULTICUT_CENTERED)
                        ni.newVar(NodeLayer.CUT_ALIGNMENT, Integer.valueOf(currentPopupIndex), ep);
                    else
			...
			if (fun == PrimitiveNode.Function.DIODE || fun == PrimitiveNode.Function.DIODEZ)
			{
				if (!currentTextField1.equals(initialTextField1))
				{
					Variable var = ni.updateVarText(Schematics.SCHEM_DIODE, currentTextField1, ep);
			...
			if (fun.isResistor())
			{
				if (!currentTextField1.equals(initialTextField1))
				{
					Variable var = ni.updateVarText(Schematics.SCHEM_RESISTANCE, currentTextField1, ep);
					if (var != null && !var.isDisplay()) ni.addVar(var.withDisplay(true));
					changed = true;
				}
			}
			if (fun.isCapacitor())
			{
				if (!currentTextField1.equals(initialTextField1))
				{
					Variable var = ni.updateVarText(Schematics.SCHEM_CAPACITANCE, currentTextField1, ep);
					if (var != null && !var.isDisplay()) ni.addVar(var.withDisplay(true));
					changed = true;
				}
			}
			if (fun == PrimitiveNode.Function.INDUCT)
			{
				if (!currentTextField1.equals(initialTextField1))
				{
					Variable var = ni.updateVarText(Schematics.SCHEM_INDUCTANCE, currentTextField1, ep);
					if (var != null && !var.isDisplay()) ni.addVar(var.withDisplay(true));
					changed = true;
				}
			}
			if (np == Schematics.tech().bboxNode)
			{
				if (!currentTextField1.equals(initialTextField1))
				{
					Variable var = ni.updateVarText(Schematics.SCHEM_FUNCTION, currentTextField1, ep);
					if (var != null && !var.isDisplay()) ni.addVar(var.withDisplay(true));
					changed = true;
				}
			}
			if (np == Schematics.tech().globalNode)
			{
				if (!currentTextField1.equals(initialTextField1))
				{
					Variable var = ni.updateVarText(Schematics.SCHEM_GLOBAL_NAME, currentTextField1, ep);
					if (var != null && !var.isDisplay()) ni.addVar(var.withDisplay(true));
					changed = true;
				}
			...
			if (np instanceof PrimitiveNode && ((PrimitiveNode)np).isPartialCircle())
			{
				if (!currentTextField1.equals(initialTextField1))
				{
					...
					ni.setArcDegrees(start, curvature, ep);
					changed = true;
				}
			}
			...
			// The following code is specific for transistors, and uses the X/Y size fields for Width and Length
			// therefore it may override the values such that the node size does not get set by them.
			...
			if (size != null)
			{
					...
					np == Schematics.tech().resistorNode)
				{
					...
					if (ni.getFunction().isFET() || ni.getFunction().isComplexResistor())
					{
						...
						double w = TextUtils.atofDistance(currentXSize, tech);
						if (w == 0)
						{
				...
				dOrient != Orientation.IDENT || changed)
			{
				...
				if (points != null)
				{
					// node described with an outline: adjust the outline
                    assert ni.getOrient() == Orientation.IDENT;
					double percX = currXSize / initXSize;
					double percY = currYSize / initYSize;
                    EPoint currentPos = EPoint.fromLambda(currentXPos, currentYPos);
				...
				{
					// note NOT described with an outline: just adjust the location and size
					...
					CircuitChangeJobs.rotateNodeText(ni, dOrient, wasMirroredInX, wasMirroredInY);
				}
			}
	...
	{//GEN-HEADEREND:event_seeActionPerformed
        ...
        PortInst pi = portObjects.get(currentIndex);
        ArcInst ai = portArcs.get(currentIndex);
		if (pi == null && ai == null) return;
		NodeInst ni = shownNode;
		if (wnd != null) {
			Highlighter highlighter = wnd.getHighlighter();
			highlighter.clear();
			if (pi != null)
				highlighter.addElectricObject(pi, ni.getParent()); else
